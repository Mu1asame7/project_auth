...
from src.repo.mappers.mappers import RoleMapper, RolesDataMapper, UserRolesMapper
...
class UserRolesRepository(BaseRepo):
    ...
    async def get_roles_by_user_id(self, user_id: int) -> list[RoleInDB]:
        stmt = select(RolesORM).join(UserRoles, UserRoles.role_id == RolesORM.id).where(UserRoles.user_id == user_id)
